...
createAnchorPeerUpdate() {
  ...
  if [ $ORG -eq 1 ]; then
    HOST="peer0.manufacturer.org.com"
    PORT=7051
  elif [ $ORG -eq 2 ]; then
    HOST="peer0.dealer.org.com"
    PORT=9051
  elif [ $ORG -eq 3 ]; then
    HOST="peer0.wholesaler.org.com"
    PORT=11051
  else
    errorln "$ORG unknown"
  fi
...
updateAnchorPeer() {
  peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.org.com -c $CHANNEL_NAME -f ${TEST_NETWORK_HOME}/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
