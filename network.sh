...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Dealer Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-dealer.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.manufacturer.org.com manufacturer
    peer_cert admin Admin@manufacturer.org.com manufacturer

    infoln "Creating Dealer Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.dealer.org.com dealer
    peer_cert admin Admin@dealer.org.com dealer

    infoln "Creating Wholesaler Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.wholesaler.org.com wholesaler
    peer_cert admin Admin@wholesaler.org.com wholesaler

    infoln "Creating Orderer Org Identities"
    #function_name cert-type   CN   
    orderer_cert orderer orderer.org.com
    orderer_cert admin Admin@org.com
...
# Create certificates for peers and orderers using fabric-ca
...
# call the script to create the channel, join the peers of manufacturer and dealer,
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    # ${CONTAINER_CLI} volume rm docker_orderer.org.com docker_peer0.manufacturer.org.com docker_peer0.dealer.org.com docker_peer0.wholesaler.org.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/dealer/msp organizations/fabric-ca/dealer/tls-cert.pem organizations/fabric-ca/dealer/ca-cert.pem organizations/fabric-ca/dealer/IssuerPublicKey organizations/fabric-ca/dealer/IssuerRevocationPublicKey organizations/fabric-ca/dealer/fabric-ca-server.db'
...
if [ "$MODE" == "prereq" ]; then
  ...
  createChannel $BFT
elif [ "$MODE" == "createCerts" ]; then
  infoln "Creating certificates"
  createCertificates
elif [ "$MODE" == "down" ]; then
